...
const int NWEEKS = 4 * 52; //there are 52 weeks in a year, so this is for 4 years, 208 weeks

struct Store
{
    IdType id;
    double total; //holds the total number of sales made in NWEEKS
    double weeklySales[NWEEKS];
    Store(IdType i) : id(i) //when we create a store, we must define and ID
    {
...
int main()
{
    ...
    cin >> nstores;
    //if input isn't valid, then terminate the program
    if ( !cin  ||  nstores <= 0)
    {
    ...
    for (size_t j = 0; j < nstores; j++)
      ...
      //randomize the ids so that we can do a sort on them
    ...
    vector<Store> stores(unorderedStores);//duplicate the vector of stores
    timer.start();
    
    //call the stl sort function and pass it the compareStore function so it can use
    //it to determine what order it should sort it in.
    sort(stores.begin(), stores.end(), compareStore);
    //report the results from the sort
    ...
    assert(is_sorted(stores.begin(), stores.end(), compareStore));  // from <algorithm>
      ...
      // fast. This is to test speed on an already sorted array
    timer.start();
    sort(stores.begin(), stores.end(), compareStore);
    report("STL sort if already sorted", timer.elapsed(), stores);
    assert(is_sorted(stores.begin(), stores.end(), compareStore));

    if (TEST_INSERTION_SORT)//if true perform the Insertion Sort
    {
...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    for(int k = 0; k < store_ptr.size(); k++)
    {
        stores[k] = *store_ptr[k];
    }
